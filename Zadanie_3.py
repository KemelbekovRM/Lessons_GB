...
class Worker:
    def __init__(self, name, surname, position, wage, bonus):
        ...
        self._income = {'profit': wage, "bonus": bonus}  # защищенный


class Position(Worker):
    def get_full_name(self):
        return f"FIO: {self.name} {self.surname}"

    def get_total_income(self):
        return f"{sum(self._income.values())} $"


BigBoss = Position("Rust", 'Hrust', "BiggBoss", 90000, 5000)
print(BigBoss.get_full_name())
print(f"Doljnost: {BigBoss.position}")
print(BigBoss.get_total_income())